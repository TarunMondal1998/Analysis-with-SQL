...
-- Inserting values in band tables
...
-- Inserting values in albums table
...
-- Names of all Bands from the songs table
SELECT bands.name FROM Songs
LEFT JOIN albums ON Songs.album_id = albums.id
LEFT JOIN bands ON albums.band_id = bands.id
GROUP BY bands.name;

-- Oldest album
SELECT name as album_name, release_year FROM albums
WHERE release_year IS NOT NULL
GROUP BY name, release_year
ORDER BY release_year ASC LIMIT 1;

-- Name of all bands that have albums
SELECT bands.name FROM albums
INNER JOIN bands ON albums.band_id = bands.id
WHERE bands.name IS NOT NULL
GROUP BY bands.name;

-- Name of all bands that do not have albums
SELECT bands.name FROM albums
INNER JOIN bands ON albums.band_id = bands.id
WHERE bands.name IS NULL
GROUP BY bands.name;

-- Longest album
SELECT albums.name, ROUND (sum(Songs.length), 2) AS Total_album_length FROM albums
INNER JOIN songs ON albums.id = Songs.album_id
GROUP BY albums.name
ORDER BY Total_album_length DESC LIMIT 1;

-- Inserting records for favourite band and one album
INSERT INTO bands (id, name)
VALUES (8, 'Coldplay');
SELECT * FROM bands;

INSERT INTO albums (id, name, release_year, band_id)
VALUES (19, 'A Head Full of Dreams', 2015, 8);
SELECT * FROM albums;

-- Delete the Band and Album
SET SQL_SAFE_UPDATES = 0;
DELETE FROM albums WHERE band_id = 8;
SELECT * FROM albums;

-- Average length of all songs
SELECT ROUND (AVG(length), 2) as Average_length_minutes FROM Songs;

-- Longest Songs from each albums
SELECT a.name AS album_name,s.name AS song_name, ROUND (s.length, 2) AS song_length
FROM albums a
INNER JOIN songs s ON a.id = s.album_id
WHERE s.length = (SELECT MAX(s2.length) FROM songs s2 WHERE s2.album_id = a.id);

-- Number songs for each band
SELECT bands.name, COUNT(songs.name) AS number_of_songs FROM songs
INNER JOIN albums ON songs.album_id = albums.id
INNER JOIN bands ON albums.band_id = bands.id
GROUP BY bands.name
ORDER BY COUNT(songs.name) DESC;

-- Addition of Decade Column
SELECT id, name, release_year, ROUND (release_year/(10), 0) AS Decade, band_id FROM albums;

-- Albums which start with the word 'The'
SELECT * FROM albums
WHERE name LIKE 'The %';

-- Albums which released in 2008 to 2013
SELECT * FROM albums
HAVING release_year  BETWEEN 2008 AND 2013;
